...
const TodoList = () => {
  ...
  async function addTodos (todo:todo) {
    const axiosAddTodo = await axios.post('http://localhost:4000/todos', todo)

    const responseTodo = await axios.get('http://localhost:4000/todos', 
    { 
      params: {
        _page: 1,
        _limit: 1
  ...
  async function deleteTodos (todoID: number) {
    const axiosAddTodo = await axios.delete(`http://localhost:4000/todos/${todoID}`)
